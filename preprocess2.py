...
def opt(budget, choix, vals=[10, 6]):
    ...
    fonction_objectif = LpAffineExpression(
        e=pu * valeurs['nombre de personnes'][0] + pv + art + deco + sal + prx * valeurs['part'][0])
    probleme.setObjective(fonction_objectif)

    # Ajouter contraintes budgéétaire
    # print(f'Variables : {variables}')
    contrainte_budget = LpConstraint(
        e=pu * valeurs['nombre de personnes'][0] + pv + art + deco + sal + prx * valeurs['part'][0],
        sense=LpConstraintEQ, name='contrainte_budgétaire', rhs=budget)
    ...
    contrainte = LpConstraint(e=n, sense=LpConstraintGE, name='great contrainte ' + 'nombre de personnes',
                              rhs=valeurs['nombre de personnes'][0])
    probleme.add(contrainte)
    contrainte_ = LpConstraint(e=n, sense=LpConstraintLE, name='less contrainte ' + 'nombre de personnes',
                               rhs=valeurs['nombre de personnes'][1])
    probleme.add(contrainte_)
    ...
    repartitions = {
        ...
        "Restauration": {"Prix Unitaire": pu.value(), "Invités": n.value(), "Prix total": pu.value() * n.value()},
        "Salle": sal.value(),
        "Gâteau": {"Prix Unitaire": prx.value(), "Parts": prt.value(), "Prix total": prx.value() * prt.value()}
    }
